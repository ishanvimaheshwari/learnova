...
import { X, CheckCircle2, ShieldCheck, Mail, Sparkles, LogIn, ArrowRight } from "lucide-react";
...
export const GoogleAuthModal: React.FC<GoogleAuthModalProps> = ({
  ...
  const handleInstantGoogleSignIn = () => {
    // Standard Google account sign in with user email
    onSignInWithGoogle({
      ...
      <div className="relative w-full max-w-md bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
        ...
        <div className="p-6 space-y-5">
          {currentUser && currentUser.provider === "google" ? (
            <div className="space-y-4">
              ...
              <div className="flex gap-2.5 pt-2">
                <button
                  ...
                  className="flex-1 py-2.5 px-4 rounded-xl border border-red-200 dark:border-red-900/50 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/30 text-xs font-bold transition"
                >
                  Sign Out
                </button>
                <button
                  onClick={() => setShowCustomInput(!showCustomInput)}
                  className="flex-1 py-2.5 px-4 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-bold transition"
                >
                  Switch Account
                </button>
              </div>
            ...
            <div className="space-y-4">
              <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
                Sign in with your Google account to automatically back up your study schedule, digital textbooks,
                annotated notes, and AI test scores.
              </p>

              {/* Primary Google Sign In Button */}
              <button
                onClick={handleInstantGoogleSignIn}
                className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-xl bg-white hover:bg-slate-50 text-slate-800 border-2 border-slate-300 hover:border-slate-400 font-semibold text-sm shadow-sm transition active:scale-[0.99] cursor-pointer"
              >
              ...
              <div className="flex items-center gap-2 my-3">
                ...
                <span className="text-[11px] text-slate-400 font-medium uppercase tracking-wider">or custom login</span>
                <div className="flex-1 h-px bg-slate-200 dark:bg-slate-800" />
              </div>
              ...
              {!showCustomInput ? (
                <button
                  ...
                  className="w-full py-2.5 px-4 text-xs font-semibold text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition flex items-center justify-center gap-1.5"
                >
                ...
                <form onSubmit={handleCustomGoogleSignIn} className="space-y-3 pt-1">
                  ...
                  <button
                    type="submit"
                    className="w-full py-2 px-4 rounded-xl bg-slate-900 dark:bg-slate-800 hover:bg-slate-800 dark:hover:bg-slate-700 text-white text-xs font-bold transition flex items-center justify-center gap-1.5"
                  >
          ...
          <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center gap-2 text-[11px] text-slate-500 dark:text-slate-400">
            ...
            <span>Encrypted OAuth session with verified Google credentials.</span>
          </div>
        </div>
      </div>
    </div>
